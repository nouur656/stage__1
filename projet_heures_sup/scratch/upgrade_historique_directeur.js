...
function modifyDirecteur() {
  ...
  const navAdd = `
    <div class="nav-tabs">
      <button id="tab-saisie" class="nav-tab active" onclick="switchPage('saisie')">✏️ Saisies du mois</button>
  ...
  // Wrap main layout
  html = html.replace('<div class="main-layout">', '<div id="page-saisie" style="display:block;">\n<div class="main-layout">');
  html = html.replace('<!-- ── Modal Ajout Enseignant ── -->', '</div>\n<!-- ── Modal Ajout Enseignant ── -->');
  ...
  `;
  html = html.replace('<!-- ── Modal Ajout Enseignant ── -->', histPageHTML + '\n<!-- ── Modal Ajout Enseignant ── -->');
  ...
  const histJS = `
    ...
    async function loadHistoriquePage() {
      ...
      try {
        ...
        // Du plus récent au plus ancien (Assuming data comes mostly sorted, we just take the object keys and reverse them)
        const monthKeys = Object.keys(grouped).reverse();
        
        let htmlContent = '';
        const monthNames = {'Janvier':1, 'Février':2, 'Mars':3, 'Avril':4, 'Mai':5, 'Juin':6, 'Juillet':7, 'Août':8, 'Septembre':9, 'Octobre':10, 'Novembre':11, 'Décembre':12};
  ...
  fs.writeFileSync('public/directeur.html', html, 'utf8');
}

modifyDirecteur();
console.log('directeur modified successfully');
