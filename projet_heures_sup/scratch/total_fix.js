...
// The file has a duplicate mess. Let's find exactly `app.get('/api/export/gid'`
const gidStart = content.indexOf("app.get('/api/export/gid'");
// Let's find the excel route start
const excelStart = content.indexOf("app.get('/api/export/excel'");

if (gidStart < 0 || excelStart < 0) {
    console.error('Cannot find boundaries', {gidStart, excelStart});
    process.exit(1);
}

// Slice out the top part before GID
const topPart = content.slice(0, gidStart);
// Slice out the bottom part from excel on
const bottomPart = content.slice(excelStart);

const perfectGidRoute = `app.get('/api/export/gid', requireAuth, requireDirection, async (req, res) => {
...
const newTotalContent = topPart + perfectGidRoute + bottomPart;
fs.writeFileSync('server.js', newTotalContent, 'utf8');
